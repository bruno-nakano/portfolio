...
export function PotteryContent({ activeTags = [], onTagClick }: ContentProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#000000', overflow: 'hidden' }}>
      {/* Header */}
      <div style={{ padding: '12px 16px 8px', fontFamily: "'Inter', 'Helvetica Neue', Helvetica, Arial, sans-serif", fontSize: '13px', color: '#ffffff', lineHeight: 1.6, flexShrink: 0, borderBottom: '1px solid #222' }}>
        <span style={{ fontWeight: '600', textTransform: 'uppercase', letterSpacing: '-0.01em' }}>Studio Mano</span>
        {' — '}
...
export function IndexContent({ onProjectClick }: { onProjectClick?: (sectionId: string, projectId?: string) => void }) {
  ...
  // INTERACTIVE is last per design
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {section.projects.map((project) => {
              ...
              return (
                <div key={project.id}>
                  {/* Project title row */}
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px', marginBottom: isExpanded ? '0' : '8px' }}>
                    <div
                      style={{
                        fontFamily: "'Inter', 'Helvetica Neue', Helvetica, Arial, sans-serif",
                        fontSize: '11px',
